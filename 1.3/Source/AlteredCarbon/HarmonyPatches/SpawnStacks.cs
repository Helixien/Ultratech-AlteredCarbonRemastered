...
namespace AlteredCarbon
{
    ...
    internal static class SpawnStacks_Patch
    {
        public static void Postfix(CompRottable __instance, RotStage __result)
        {
            if (__result == RotStage.Dessicated && __instance.parent is Corpse corpse
                && (corpse.InnerPawn?.health?.hediffSet?.HasHediff(AC_DefOf.AC_CorticalStack) ?? true))
            {
                try
                {
                    ...
                    corpse.InnerPawn.health.hediffSet.hediffs.RemoveAll(x => x.def == AC_DefOf.AC_CorticalStack);
                }
                catch { }
            }
        }
    ...
    internal static class DoFireDamage_Patch
    {
        ...
        {
            if (targ is Corpse corpse && targ.HitPoints <= 3 && (corpse.InnerPawn?.health?.hediffSet?.HasHediff(AC_DefOf.AC_CorticalStack) ?? true))
            {
                var corticalStack = ThingMaker.MakeThing(AC_DefOf.AC_FilledCorticalStack) as CorticalStack;
                corticalStack.PersonaData.CopyPawn(corpse.InnerPawn);
                ...
                && (pawn.health?.hediffSet?.HasHediff(AC_DefOf.AC_CorticalStack) ?? true))
            {
                var corticalStack = ThingMaker.MakeThing(AC_DefOf.AC_FilledCorticalStack) as CorticalStack;
                corticalStack.PersonaData.CopyPawn(pawn);
                GenPlace.TryPlaceThing(corticalStack, pawn.Position, pawn.Map, ThingPlaceMode.Direct);
                pawn.health.hediffSet.hediffs.RemoveAll(x => x.def == AC_DefOf.AC_CorticalStack);
                __instance.Destroy(DestroyMode.Vanish);
            }
        }
    }
