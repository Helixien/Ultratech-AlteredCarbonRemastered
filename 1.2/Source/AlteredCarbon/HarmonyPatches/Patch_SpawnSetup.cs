        ...
        public static void Postfix(Pawn __instance, Map map, bool respawningAfterLoad)
        {
            if (!respawningAfterLoad && __instance.RaceProps.Humanlike && __instance.kindDef.HasModExtension<StackSpawnModExtension>())
            {
                ...
                if (extension.SpawnsWithStack && Rand.Chance((float)extension.ChanceToSpawnWithStack / 100f))
                {
                    ...
                    var hediff = HediffMaker.MakeHediff(AlteredCarbonDefOf.AC_CorticalStack, __instance, neckRecord) as Hediff_CorticalStack;
                    hediff.gender = __instance.gender;
                    hediff.race = __instance.kindDef.race;
                    ...
                    ACUtils.ACTracker.RegisterPawn(__instance);
                    ACUtils.ACTracker.TryAddRelationships(__instance);
                }
            }
        }
    }
