...
namespace AlteredCarbon
{
    public class JobGiver_TakeStackWhenClose : ThinkNode_JobGiver
    {
        protected override Job TryGiveJob(Pawn pawn)
        {
            ...
            var corticalStacks = pawn.Map.listerThings.ThingsOfDef(AlteredCarbonDefOf.AC_FilledCorticalStack).Cast<CorticalStack>()
                .Where(x => x.faction == pawn.Faction && x.Position.DistanceTo(pawn.Position) < 10);
