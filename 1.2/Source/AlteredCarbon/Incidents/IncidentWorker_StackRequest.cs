	...
	internal class IncidentWorker_StackRequest : IncidentWorker
	{
		...
		protected override bool TryExecuteWorker(IncidentParms parms)
		{
            ...
            if (stacks?.Any() ?? false)
            {
                ...
                DiaNode diaNode = new DiaNode("AC.FactionDemandsStacks".Translate(string.Join(", ", stacks.Select(x => x.name)), faction.Named("FACTION")));
        ...
        protected void SpawnPawns(IncidentParms parms, List<CorticalStack> stacks)
        {
            ...
            while (list.Count < minimumPawnCount)
            {
                ...
                foreach (Pawn item in temp)
                {
                    ...
                    item.mindState.duty = new PawnDuty(AlteredCarbonDefOf.AC_TakeStacks, stacks.RandomElement().Position);
                }
                list.AddRange(temp);
            }
        ...
        {
            var corticalStacks = map.listerThings.ThingsOfDef(AlteredCarbonDefOf.AC_FilledCorticalStack).Cast<CorticalStack>()
                .Where(x => x.faction != null && x.faction.def.humanlikeFaction && !x.faction.IsPlayer && x.faction.RelationKindWith(Faction.OfPlayer) != FactionRelationKind.Hostile);
            ...
            {
                if (stack.faction != null && stack.faction.def?.pawnGroupMakers != null && !map.mapPawns.PawnsInFaction(stack.faction).Any())
                {
                    if (factionCandidates.ContainsKey(stack.faction))
                    {
                        factionCandidates[stack.faction].Add(stack);
                    }
                    else
                    {
                        factionCandidates[stack.faction] = new List<CorticalStack> { stack };
                    }
                }
            }
